...
alter table public.workspaces
  add column invite_code_seed uuid,
  add column creation_request_id uuid;

create unique index workspaces_creation_request_unique
...
begin
  ...
  if current_role is null then
    current_role := 'PLAYER';
    insert into public.workspace_members(workspace_id, user_id, role)
    values (target_workspace_id, target_user_id, current_role);
  end if;
  ...
  return query select current_role, linked_player_id;
end;
$$;
